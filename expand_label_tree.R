library(data.table)
dat = data.table(read.csv("~/consensys/labels.csv"))
...
getRecursiveLabels = function(tax, label){
  ...
  result = NULL
  print(paste0(c("cat: ",categories[highLevelInd])))
  len = length(highLevelInd)
  if (len>0){
    newLabels= tax[highLevelInd,2:ncol(tax)]
    newLabels = newLabels[which(newLabels!="")]
    ...
    print(paste0("no high level cat found, returning original label: ",label))
    result = label
  }
...
for (i in 1:nrow(tax)){
  ...
  for (j in 1:length(thisRow)){
    ...
    synonyms=syn[ind[1],]
    synonyms = synonyms[which(synonyms!="")]
